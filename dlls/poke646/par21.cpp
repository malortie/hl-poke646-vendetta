...
void CPar21::Precache( void )
{
	...
	PRECACHE_SOUND("weapons/par21_reload.wav");

	PRECACHE_SOUND ("weapons/par21_1.wav"); // H to the K
	PRECACHE_SOUND ("weapons/par21_2.wav"); // H to the K
	PRECACHE_SOUND ("weapons/par21_3.wav"); // H to the K
...
BOOL CPar21::Deploy( )
{
	return DefaultDeploy( "models/v_par21.mdl", "models/p_par21.mdl", PAR21_DEPLOY, "mp5" );
}
...
void CPar21::SecondaryAttack( void )
{
	...
	PLAYBACK_EVENT( flags, m_pPlayer->edict(), m_usM203 );

	m_flNextPrimaryAttack = GetNextAttackDelay(0.9);
	m_flNextSecondaryAttack = GetNextAttackDelay(0.9);
...
{
	if ( m_pPlayer->ammo_nato <= 0 )
		return;

	DefaultReload( PAR21_MAX_CLIP, PAR21_RELOAD, 1.5 );
}
...
class CM203AmmoGrenade : public CBasePlayerAmmo
{
	void Spawn( void )
	{
		Precache( );
...
LINK_ENTITY_TO_CLASS( ammo_par21_grenade, CM203AmmoGrenade );
LINK_ENTITY_TO_CLASS( ammo_m203grenade, CM203AmmoGrenade );
